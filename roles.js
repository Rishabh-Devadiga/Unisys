...
function authLogin(email, password, role) {
  ...
  storeSet(SESSION_KEY, JSON.stringify({
    email: user.email, role: user.role, name: user.name, dept: user.dept, title: user.title
  }));
...
var ROLE_MODULES = {
  ...
  Principal: [
    'overview','students','academics','attendance','exams','faculty',
    'finance','placements','research','compliance','analytics','attainment',
    'communications','coverage',
    /* Principal-only extras */
    'role-perf','role-proposals','role-strategic'
  ],
  HOD: [
    'overview','students','academics','exams','faculty',
    'compliance','analytics','communications',
    /* HOD-only extras */
    'role-dept','role-timetable','role-leave','role-marks'
  ],
  Faculty: [
    'overview',
    /* Faculty-only */
...
var ROLE_NAV = {
  Admin: [
    ...
    { id:'role-config', icon:'⚙️',  label:'System Config',       section:'Admin Tools' },
...
function showRoleBadge(sess) {
  ...
  if (actions && !actions.querySelector('.logout-btn')) {
    ...
    logoutBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>';
...
function buildRoleCard(role, icon, email, pass, color, desc) {
  return '<div class="integration-card '+(color==='green'?'active-card':'')+'" style="flex-direction:column;align-items:flex-start;gap:8px;cursor:pointer" onclick="loginAsRole(\''+role+'\') ">'
    ...
    + '<button class="btn btn-sm btn-primary" style="width:100%;margin-top:2px" onclick="loginAsRole(\''+role+'\');event.stopPropagation()">Login as '+role+' →</button>'
    + '</div>';
}
